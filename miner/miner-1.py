...
repo_git = "https://github.com/angular/angular.js.git"
extensions = [".js", ".java"]
keywords = ["fix", "bug", "miss"]
...
def miner():
    ...
    if not os.path.exists(bic_csv):
        ...
        writer.writeheader()
        to_date = datetime(2018, 6, 1, 12, 0, 0)
        ...
        for commit in RepositoryMining(repo_path, to=to_date, reversed_order=True).traverse_commits():
            ...
            if any(word in msg for word in keywords):
                ...
                no_fix_count += 1
                dout = {"hash": commit.hash, "size": len(mods), "developer": commit.author.email, "fix": False, "bic_path": "---", "bic_hash": "---", "bic_size": "---"}
                for mod in mods:
                    dout["path"] = mod.new_path
                    writer.writerow(dout)
                    out_file.flush()
    ...
    ratio_partially_defective_commits = round((partially_defective / fully_defective) * 100, 1)
    print("Partially def. commits: {}% {} {}. Defective files in partially def. commits: {}%".format(ratio_partially_defective_commits, partially_defective, fully_defective, ratio_defective_files_in_defective_commits))
